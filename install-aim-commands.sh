#!/bin/bash
# Manual installation script for aim-antenna and aim-simple commands
# Run this directly on the Pi to install the fixed commands

set -e

SATPI_DIR="/home/johnc/satpi"

echo "🛰️  Installing Fixed aim-antenna and aim-simple Commands"
echo "========================================================"
echo ""

# Check if we're running on the Pi
if [[ ! -d "$SATPI_DIR" ]]; then
    echo "❌ Error: SatPi directory not found at $SATPI_DIR"
    echo "This script should be run on the Pi with SatPi installed."
    exit 1
fi

# Create aim-antenna command
echo "📡 Creating aim-antenna command..."
cat > "$SATPI_DIR/aim-antenna" << 'EOF'
#!/bin/bash
...
# Create aim-simple command
echo "📡 Creating aim-simple command..."
cat > "$SATPI_DIR/aim-simple" << 'EOF'
#!/bin/bash
...
chmod +x "$SATPI_DIR/aim-antenna"
chmod +x "$SATPI_DIR/aim-simple"

# Install to /usr/local/bin for system-wide access
echo "🔧 Installing commands system-wide..."
sudo cp "$SATPI_DIR/aim-antenna" /usr/local/bin/
sudo cp "$SATPI_DIR/aim-simple" /usr/local/bin/
...
echo ""
echo "The commands now work properly for GOES signal strength scanning"
echo "and will no longer return straight to the prompt." 